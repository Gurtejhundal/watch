...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground selection:bg-accent selection:text-white pb-32">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-24 md:pt-32 grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16">
        {/* Gallery */}
        ...
        <div className="relative">
          <div className="lg:sticky lg:top-32">
            ...
            <h1 className="text-3xl md:text-5xl font-serif mb-4 md:mb-6 leading-tight">{product.name}</h1>
            <p className="text-lg md:text-xl font-medium mb-6 md:mb-8">${parseFloat(product.price).toLocaleString()}</p>
            
            <p className="text-foreground/70 leading-relaxed mb-8 md:mb-12 text-xs md:text-sm">
              {product.description}
            </p>

            <div className="flex flex-col gap-3 md:gap-4 mb-8 md:mb-12">
              <button className="w-full bg-foreground text-background py-3.5 md:py-4 uppercase tracking-widest text-[10px] md:text-xs hover:bg-foreground/90 transition-colors">
                Add to Cart
              </button>
              <button className="w-full bg-transparent border border-metal/50 text-foreground py-3.5 md:py-4 uppercase tracking-widest text-[10px] md:text-xs hover:border-foreground transition-colors">
                Add to Wishlist
              </button>
            </div>

            {/* Technical Specifications */}
            <div className="border-t border-metal/20 pt-6 md:pt-8">
              <h3 className="text-[10px] md:text-xs uppercase tracking-widest font-semibold mb-4 md:mb-6">Technical Specifications</h3>
